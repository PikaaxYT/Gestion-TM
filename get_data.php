...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
		...
		'MES' => !empty($row['MES']) && $row['MES'] != '0000-00-00' ? date('d-m-Y', strtotime($row['MES'])) : '?',
		'Garantie' => !empty($row['Garantie']) && $row['Garantie'] != '0000-00-00' ? date('d-m-Y', strtotime($row['Garantie'])) : 'Non',
		'CT' => !empty($row['CT']) && $row['CT'] != '0000-00-00' ? date('d-m-Y', strtotime($row['CT'])) : '?',
            'Revision' => !empty($row['Revision']) && $row['Revision'] != '0000-00-00' ? date('d-m-Y', strtotime($row['Revision'])) : 'Jamais',
